    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        if !self.is_db_up.load(Ordering::Relaxed) {
            let err = GenericServiceError {
                error: GenericServiceErrorDetails {
                    code: UserIdentityServiceError::ApplicationCurrentlyUnavailable,
                    code_numeric: 500,
        ...
        if !self.is_broker_up.load(Ordering::Relaxed) {
            if path.starts_with ("/api/v1/auth/register") || path.starts_with("/api/v1/auth/anonymous"){
                let err = GenericServiceError {
                    error: GenericServiceErrorDetails {
                        code: UserIdentityServiceError::ApplicationCurrentlyUnavailable,
                        code_numeric: 500,
