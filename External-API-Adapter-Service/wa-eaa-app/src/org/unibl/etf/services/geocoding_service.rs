...
impl GeocodingService {
    ...
    pub async fn geocode_location(
        ...
        limit: u8
    ) -> Result<(f64, f64), AdapterError> {//mozda reprezentovati koordinate ne kao tuple!!!
        let response = client
        ...
        if response.status().is_success() {
            let body_text = response.text().await.map_err(|e| {
                AdapterError::ConnectionError(format!("Failed to get response body text: {}", e))
            })?;
            ...
            if data.len() == 0 {
                ...
                return Ok((geocoding_response.lat, geocoding_response.lon)) //mozda vratiti i state i drzavu da user i tu informaciju ima
            } else {
                let mut candidates: Vec<Candidate> = Vec::new();
                for geocoding_response in data {
                    candidates.push(geocoding_response.try_into().unwrap());
                }
                return Err(AdapterError::AmbiguousLocation("There are multiple location the with provided name. Choose one of them". to_string(), candidates))
            }
        }
        else {
